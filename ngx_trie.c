...
ngx_trie_node_t* ngx_trie_insert(ngx_trie_t* trie, ngx_str_t* str, ngx_uint_t mode) {
    size_t i;
    ...
    while (i < str->len) {
        ...
        if (p->next == NULL) {
            p->next = ngx_pcalloc(trie->pool, NGX_TRIE_KIND * sizeof(ngx_trie_node_t*));
...
void* ngx_trie_query(ngx_trie_t* trie, ngx_str_t* str, ngx_int_t* version_pos, ngx_uint_t mode) {
    void* value;
    size_t i;
